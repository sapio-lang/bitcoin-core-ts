...
exports.obfuscate = void 0; // @ts-expect-error ts-migrate(7016) FIXME: Could not find a declaration file for module 'loda... Remove this comment to see the full error message
...
const lowercaseMethods = (0, lodash_1.mapKeys)(methods_1.default, (value, key) => key.toLowerCase());
/**
...
function obfuscateResponse(request, instance) {
  ...
  if ((0, lodash_1.get)(request, `headers['content-type']`) === 'application/octet-stream') {
    request.body = '******';
    return;
  }
  ...
  if ((0, lodash_1.isArray)(request.body)) {
    const methodsById = (0, lodash_1.mapKeys)(requestBody, method => method.id);
    request.body = (0, lodash_1.map)(request.body, request => obfuscateResponseBody(request, methodsById[request.id].method));
  } else {
...
function obfuscateRequestBody(body) {
  const method = (0, lodash_1.get)(lowercaseMethods[body.method], 'obfuscate.request');
