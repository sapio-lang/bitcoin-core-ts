...
const rpc_error_1 = __importDefault(require("./errors/rpc-error")); // @ts-expect-error ts-migrate(7016) FIXME: Could not find a declaration file for module 'loda... Remove this comment to see the full error message
...
function getRpcResult(body, {
  ...
  if (body.error !== null) {
    throw new rpc_error_1.default(lodash_1.default.get(body, 'error.code', -32603), lodash_1.default.get(body, 'error.message', 'An error occurred while processing the RPC call to bitcoind'));
  } // Defensive measure. This should not happen on a RPC call.


  if (!lodash_1.default.has(body, 'result')) {
    throw new rpc_error_1.default(-32700, 'Missing `result` on the RPC call result');
  }
...
class Parser {
  ...
  rpc(response) {
    ...
    if (typeof response.body === 'string' && response.headers['content-type'] !== 'application/json' && response.statusCode !== 200) {
      throw new rpc_error_1.default(response.statusCode, response.statusMessage, {
        body: response.body
      });
  ...
  rest(extension, response) {
    ...
    if (response.headers['content-type'] !== 'application/json' && response.statusCode !== 200) {
      ...
      throw new rpc_error_1.default(response.statusCode, response.body.replace('\r\n', ''), {
        body: response.body
      });
    } // Parsing the body with custom parser to support BigNumbers.
