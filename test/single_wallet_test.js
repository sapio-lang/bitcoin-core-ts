...
describe("Single Wallet", () => {
    ...
    describe("getBalance()", () => {
      ...
      it("should support named parameters", async () => {
        const client = new Client(
          defaults({ version: "0.17.0" }, config.bitcoin)
        );
  ...
  describe("batched requests", () => {
    ...
    it("should support request parameters in batched requests", async () => {
      ...
      addressValidation.should.have.properties(
        "address",
        "isvalid",
        "scriptPubKey",
        "isscript",
    ...
    it("should return an error if one of the request fails", async () => {
      ...
      const [validateAddressError, validateAddress] = await client.command(
        batch
      );

      validateAddress.should.have.properties(
        "address",
        "isvalid",
        "scriptPubKey"
      );
