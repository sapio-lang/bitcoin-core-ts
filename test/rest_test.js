...
describe("REST", () => {
  ...
  describe("getTransactionByHash()", () => {
    ...
    it("should return a transaction hex-encoded if extension is `hex`", async () => {
      ...
      const hexTransaction = await client.getTransactionByHash(txid, {
        extension: "hex",
      });
    ...
    it("should return a transaction binary-encoded if extension is `bin`", async () => {
      const [{ txid }] = await client.listUnspent();
      const binaryTransaction = await client.getTransactionByHash(txid, {
        extension: "bin",
      });
      const hexTransaction = await client.getTransactionByHash(txid, {
        extension: "hex",
  ...
  describe("getBlockHeadersByHash()", () => {
    ...
    it("should return block headers binary-encoded if extension is `bin`", async () => {
      const hash =
        "0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206";
      const binaryHeaders = await client.getBlockHeadersByHash(hash, 1, {
        extension: "bin",
      });
      const hexHeaders = await client.getBlockHeadersByHash(hash, 1, {
        extension: "hex",
      });

      binaryHeaders
        .toString("hex")
        .should.equal(`${hexHeaders.toString("hex").replace("\n", "")}`);
    });
  });

  describe("getBlockchainInformation()", () => {
    it("should return blockchain information json-encoded by default", async () => {
      const information = await new Client(
        config.bitcoin
      ).getBlockchainInformation();

      information.should.have.properties(
        "bestblockhash",
        "blocks",
        "chain",
        "chainwork",
  ...
  describe("getUnspentTransactionOutputs()", () => {
    ...
    it("should return unspent transaction outputs binary-encoded if extension is `bin`", async () => {
      ...
      const binaryUnspents = await new Client(
        config.bitcoin
      ).getUnspentTransactionOutputs(outputs, { extension: "bin" });
      const hexUnspents = await new Client(
        config.bitcoin
      ).getUnspentTransactionOutputs(outputs, { extension: "hex" });
